...
export default function Chatbot() {
  ...
  return (
    <main
      ref={ref}
      className="relative flex flex-col w-full h-[40vw] overflow-auto bg-transparent p-4 shadow bg-gradient-to-r from-gray-600 to-gray-900 text-white shadow-xl p-6"
    ...
    >
      ...
      <section className="mb-4 flex-grow overflow-auto">
        {messages.map((m) => (
          <div
            className={`mb-2 p-2 rounded-lg ${m.role === "user" ? "bg-blue-500 text-white" : "bg-grey-200 text-black"}`}
            key={m.id}
          >
      ...
      <form className="flex mt-auto" onSubmit={handleSubmit}>
        <input
          ...
          placeholder="Ask me: What are some of the things Ankit can do?"
        />
        <button
